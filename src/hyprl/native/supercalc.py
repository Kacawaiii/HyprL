...
# =============================================================================
# HIGH-PERFORMANCE DATA LOADING (v0.2.0)
# =============================================================================


def load_parquet_fast(path: str | Path, *, use_mmap: bool = True) -> pl.DataFrame:
    """Load a Parquet file directly via Rust (2-5x faster than Python for large files).

    Args:
        path: Path to the Parquet file.
        use_mmap: Use memory mapping for files > 1GB (recommended).

    Returns:
        Polars DataFrame with columns: ts, open, high, low, close, volume.
    """
    if not native_available():
        raise NativeEngineUnavailable("hyprl_supercalc is not installed")

    path_str = str(path)
    candles = _native.load_parquet_py(path_str, use_mmap)  # type: ignore[attr-defined]

    # Convert list of candle dicts to Polars DataFrame
    if not candles:
        return pl.DataFrame(schema={"ts": pl.Int64, "open": pl.Float64, "high": pl.Float64,
                                    "low": pl.Float64, "close": pl.Float64, "volume": pl.Float64})

    return pl.DataFrame(candles)


def get_parquet_info(path: str | Path) -> dict[str, object]:
    """Get Parquet file info without loading all data.

    Args:
        path: Path to the Parquet file.

    Returns:
        Dict with keys: path, file_size_bytes, file_size_mb, row_count, columns.
    """
    if not native_available():
        raise NativeEngineUnavailable("hyprl_supercalc is not installed")

    return _native.get_parquet_info_py(str(path))  # type: ignore[attr-defined]


def run_fast_backtest_from_parquet(
    path: str | Path,
    signal: Sequence[float],
    cfg: BacktestConfig,
    *,
    ...
    """Run backtest directly from Parquet file (zero-copy, fastest path).

    Args:
        path: Path to the Parquet file.
        signal: Trading signal array (must match row count).
        cfg: Backtest configuration.
        use_mmap: Use memory mapping for large files.
        use_atr_position_sizing: Override ATR position sizing.

    Returns:
        Raw backtest report dict (use for speed, not full BacktestResult).
    """
    if not native_available():
        raise NativeEngineUnavailable("hyprl_supercalc is not installed")

    config_dict = _config_to_native_dict(cfg, use_atr_position_sizing=use_atr_position_sizing)
    signal_vec = [float(v) for v in signal]

    return _native.run_backtest_from_parquet_py(str(path), signal_vec, config_dict, use_mmap)  # type: ignore[attr-defined]
...
def run_multi_symbol_backtest(
    ...
    *,
    ...
    """Run parallel backtests on multiple symbols.

    Args:
        data: Dict mapping symbol -> flat candle data (ts, o, h, l, c, v repeated).
        signals: Dict mapping symbol -> signal array.
        cfg: Backtest configuration (shared across symbols).
        use_atr_position_sizing: Override ATR position sizing.

    Returns:
        MultiSymbolReport with individual reports and aggregate metrics.
    """
    if not native_available():
        raise NativeEngineUnavailable("hyprl_supercalc is not installed")
    ...
    data_native = {sym: [float(v) for v in vals] for sym, vals in data.items()}
    signals_native = {sym: [float(v) for v in vals] for sym, vals in signals.items()}

    return _native.run_multi_backtest_py(data_native, signals_native, config_dict)  # type: ignore[attr-defined]
...
def benchmark_backtest(
    ...
    Returns:
        Dict with: iterations, total_time_ms, avg_time_ms, bars_per_second.
    """
    if not native_available():
        raise NativeEngineUnavailable("hyprl_supercalc is not installed")
    ...
    polars_df = _ensure_polars_dataframe(df)
    signal_vec = _normalize_signal(signal, polars_df.height)
    config_dict = _config_to_native_dict(cfg)

    return _native.benchmark_backtest_py(polars_df, signal_vec, config_dict, iterations)  # type: ignore[attr-defined]


def estimate_processing_time(row_count: int, n_symbols: int = 1) -> dict[str, float]:
    """Estimate processing time for a dataset.

    Based on benchmarks: ~500K bars/sec on single core, ~2M bars/sec parallel.

    Args:
        row_count: Total rows per symbol.
        n_symbols: Number of symbols.

    Returns:
        Dict with estimated times in seconds.
    """
    total_bars = row_count * n_symbols
    single_core_rate = 500_000  # bars/sec
    parallel_rate = 2_000_000  # bars/sec

    return {
        "total_bars": total_bars,
        "single_core_seconds": total_bars / single_core_rate,
        "parallel_seconds": total_bars / parallel_rate,
        "recommended": "parallel" if n_symbols > 1 or row_count > 100_000 else "single",
    }

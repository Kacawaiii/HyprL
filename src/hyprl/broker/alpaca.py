...
class AlpacaBroker(BrokerBase):
    ...
    def __init__(
        ...
        self._crypto_data_client = CryptoHistoricalDataClient(self._api_key, self._secret_key)
        ...
        self._AlpacaOrderClass = AlpacaOrderClass
        self._QueryOrderStatus = QueryOrderStatus
        self._StockLatestTradeRequest = StockLatestTradeRequest
        self._CryptoLatestTradeRequest = CryptoLatestTradeRequest
        self._CryptoLatestQuoteRequest = CryptoLatestQuoteRequest
    ...
    def submit_order(
        ...
        For crypto orders, use notional (dollar amount) instead of qty.
        Alpaca requires minimum $10 notional for crypto.
        """
        symbol = symbol.upper()
        ...
        # For crypto, use notional (dollar amount) if provided
        is_crypto = "/" in symbol
        use_notional = is_crypto and notional is not None and notional >= 10
    ...
    def submit_bracket(
        ...
        alpaca_side = self._map_order_side(side)
        alpaca_tif = self._map_time_in_force(time_in_force)
        tp_req = self._TakeProfitRequest(limit_price=float(take_profit))
        sl_req = self._StopLossRequest(stop_price=float(stop_loss))
        ...
        try:
            request = self._MarketOrderRequest(
                ...
                time_in_force=alpaca_tif,
                client_order_id=client_order_id,
                order_class=self._AlpacaOrderClass.BRACKET,
                take_profit=tp_req,
                stop_loss=sl_req,
            )
            result = self.client.submit_order(request)
            order = self._alpaca_order_to_order(result)
            logger.info(
                "Bracket order submitted: %s %s %s qty=%s tp=%s sl=%s",
                order.id,
                symbol,
                side.value,
                qty,
            ...
            raise BrokerError(f"Failed to submit bracket order: {exc}") from exc
        ...
        try:
            ...
            for order in orders:
                try:
                    ...
                    count += 1
                    logger.info("Canceled order %s for %s", order.id, symbol)
    ...
    def get_latest_trade_price(self, symbol: str) -> Optional[float]:
        """Fetch latest trade price for a symbol (best-effort)."""
        symbol = symbol.upper()
        if self._is_crypto_symbol(symbol):
            try:
                crypto_symbol = self._normalize_crypto_symbol(symbol)
                request = self._CryptoLatestTradeRequest(symbol_or_symbols=crypto_symbol)
                data = self._crypto_data_client.get_crypto_latest_trade(request)
                trade = data.get(crypto_symbol) if isinstance(data, dict) else data
                price = getattr(trade, "price", None)
                return float(price) if price is not None else None
            except Exception as exc:
                logger.warning("Failed to get latest crypto trade for %s: %s", symbol, exc)
                return None
        try:
            request = self._StockLatestTradeRequest(symbol_or_symbols=symbol)
            data = self._data_client.get_stock_latest_trade(request)
    ...
    def get_latest_quote(self, symbol: str) -> Optional[Dict[str, float]]:
        ...
        Returns:
            Dict with 'bid', 'ask', 'spread', 'spread_pct' or None on error.
        """
        symbol = symbol.upper()
        if self._is_crypto_symbol(symbol):
            try:
                crypto_symbol = self._normalize_crypto_symbol(symbol)
                request = self._CryptoLatestQuoteRequest(symbol_or_symbols=crypto_symbol)
                data = self._crypto_data_client.get_crypto_latest_quote(request)
                quote = data.get(crypto_symbol) if isinstance(data, dict) else data
                ...
                logger.warning("Failed to get latest crypto quote for %s: %s", symbol, exc)
                return None
        try:
            from alpaca.data.requests import StockLatestQuoteRequest
            request = StockLatestQuoteRequest(symbol_or_symbols=symbol)
            data = self._data_client.get_stock_latest_quote(request)
            quote = data.get(symbol) if isinstance(data, dict) else data
            bid = float(getattr(quote, "bid_price", 0) or 0)
            ask = float(getattr(quote, "ask_price", 0) or 0)
            if bid <= 0 or ask <= 0:
                return None
            ...
            logger.warning("Failed to get latest quote for %s: %s", symbol, exc)
            return None

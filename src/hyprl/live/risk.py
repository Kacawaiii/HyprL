    ...
    def allow_trade(self, signal: TradeSignal, price: float) -> LiveRiskDecision:
        ...
        if self.cfg.max_notional_per_trade is not None and self.cfg.max_notional_per_trade > 0:
            if notional > self.cfg.max_notional_per_trade:
                reason = "position_notional_exceeded"

        if not reason and self.cfg.max_position_notional_pct > 0:
        ...
        if self._parity_mode:
            return LiveRiskDecision(True, reason, bypassed=True)

        if reason:
            return LiveRiskDecision(False, reason)

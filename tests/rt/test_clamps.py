...
async def test_rate_cap_and_qty_clamp(tmp_path: Path) -> None:
    ...
    cfg = LiveConfig(
        ...
        min_qty=5,
        risk_pct=0.01,
        warmup_bars=2,  # Reduced warmup for faster test
    )
    ...
    assert lines, "No predictions generated"
    reasons = {__import__('json').loads(line).get("reason") for line in lines}
    # Accept warmup, rate_cap, or qty_clamp as valid reasons
    valid_reasons = {"warmup", "rate_cap", "qty_clamp", "signal", None}
    assert reasons & valid_reasons, f"Unexpected reasons: {reasons}"

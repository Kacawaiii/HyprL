...
async def test_queue_source_loop(tmp_path: Path) -> None:
    ...
    assert predictions.exists(), "predictions.jsonl should be created"
    ...
    # orders.jsonl may not exist if no orders were submitted during warmup
    ...
    if orders.exists():
        ...
        # Orders may be empty during warmup-only run

...
- `checkout.html` - Checkout handoff (Stripe Payment Links)
...
**Checkout has placeholders that MUST be replaced with actual Stripe Payment Links before going live.**

### Where to Paste Payment Links

Edit `apps/landing/checkout.html` and replace these exact strings:

1. **Beta tier** → replace `STRIPE_LINK_BETA`
2. **Starter tier** → replace `STRIPE_LINK_STARTER`
3. **Pro tier** → replace `STRIPE_LINK_PRO`
4. **Team tier** → replace `STRIPE_LINK_TEAM`

...
def _load_feature_list(path_str: str, *, config_path: str | None = None) -> list[str]:
    candidate = Path(path_str).expanduser()
    if not candidate.is_absolute() and config_path:
        cfg_path = Path(config_path).expanduser()
        if not cfg_path.is_absolute():
            cfg_path = (Path.cwd() / cfg_path).resolve()
        if cfg_path.is_file():
            scoped = (cfg_path.parent / candidate).resolve()
            if scoped.exists():
                candidate = scoped
    if not candidate.exists():
        raise FileNotFoundError(f"Feature list not found: {candidate}")
    payload = json.loads(candidate.read_text())
    if not isinstance(payload, list) or not all(isinstance(item, str) for item in payload):
        raise ValueError(f"Feature list must be a JSON list of strings: {candidate}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Optional calibration artifact (.joblib) applied to predicted probabilities.",
    )
    parser.add_argument(
        "--export-trades",
        type=Path,
...
def _apply_backtest_cli_config(args: argparse.Namespace) -> None:
    ...
    if isinstance(calibration_cfg, dict):
        artifact = calibration_cfg.get("artifact")
        if artifact and not getattr(args, "calibration_artifact", None):
            setattr(args, "calibration_artifact", artifact)
    else:
        _assign_if_default(args, defaults, "calibration", calibration_cfg)
    _assign_if_default(args, defaults, "seed", model_cfg.get("seed"))
    feature_list = model_cfg.get("feature_list")
    if feature_list:
        try:
            setattr(
                args,
                "model_feature_columns",
                _load_feature_list(str(feature_list), config_path=config_path),
            )
        except Exception as exc:
            raise ValueError(f"Failed to load feature_list from config: {feature_list}") from exc
    elif not getattr(args, "model_feature_columns", None):
        try:
...
def main() -> None:
    ...
    if isinstance(calibration_cfg, dict):
        calibration_artifact = calibration_cfg.get("artifact")
        calibration = str(calibration_cfg.get("method", "none"))
    else:
        calibration = args.calibration or calibration_cfg or settings.get("calibration", "none")
    if args.calibration is not None:
        calibration = args.calibration
    if args.calibration_artifact is not None:
        calibration_artifact = str(args.calibration_artifact)
    model_feature_columns = getattr(args, "model_feature_columns", None)
    if not model_feature_columns:
        feature_list = model_cfg.get("feature_list")
        if feature_list:
            model_feature_columns = _load_feature_list(str(feature_list))
        else:
            model_feature_columns = model_cfg.get("feature_columns") or []
